...
struct LocaleManager {
    ...
    static var languageCode: String {
        guard var splits = Locale.preferredLanguages.first?.split(separator: "-"), let first = splits.first else { return "" }
    ...
    static func localizedCountry(from countryCode: String) -> String {
        let id = NSLocale.localeIdentifier(fromComponents: [NSLocale.Key.countryCode.rawValue: countryCode])
        
        guard let country = NSLocale(localeIdentifier: "en").displayName(forKey: NSLocale.Key.identifier, value: id),
              let loacalizedString = NSLocale(localeIdentifier: languageCode).displayName(forKey: NSLocale.Key.identifier, value: id) else { return "" }
        return "\(loacalizedString) (\(country))"
    }
    
    static func locale(from languageCode: String?) -> Locale? {
